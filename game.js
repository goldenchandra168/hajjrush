...
function drawBackground() {
  ...
  // Sky gradient — cover FULL canvas height so no black gaps
  ...
  // Stars (phase 1 only)
  if (state.phase === 0) {
    const STAR_SEED = [173, 97, 251, 137, 61, 211, 83, 149, 199, 53,
                       127, 233, 71, 191, 113, 241, 67, 163, 107, 223,
                       89, 179, 131, 47, 197, 157, 79, 103, 167, 229];
    ctx.fillStyle = "#FFFFFF";
    for (let i = 0; i < 30; i++) {
      const sx = (STAR_SEED[i % 30] * (i + 1) * 37) % W;
      const sy = (STAR_SEED[(i + 7) % 30] * (i + 3) * 19) % (H * 0.55);
      const ss = i % 4 === 0 ? 2 : 1;
      const alpha = 0.5 + 0.5 * Math.sin(state.animFrame * 0.04 + i * 1.3);
  ...
  // Ground — draw from GROUND_Y to bottom of canvas
  ...
  const lineOffset = state.bgOffset[0] % lineSpacing;
  for (let i = -1; i < Math.ceil(W / lineSpacing) + 2; i++) {
    const lx = i * lineSpacing - lineOffset;
    ctx.beginPath();
    ctx.moveTo(lx, GROUND_Y + 20);
    ctx.lineTo(lx + 60, GROUND_Y + 20);
    ctx.stroke();
  }
}

function drawSilhouettes() {
  // Use a clean wrap: offset scrolls right-to-left, tile width = W
  const rawOffset = state.bgOffset[1] * 0.3;
  const tileW = W;
  // We draw 2 tile copies so there's no seam
  for (let copy = -1; copy <= 1; copy++) {
    const offset = -(rawOffset % tileW) + copy * tileW;
    ctx.fillStyle = "rgba(0,0,0,0.4)";

    if (state.phase === 0) {
      drawMosque(offset + tileW * 0.1, GROUND_Y - 100, 80);
      drawMosque(offset + tileW * 0.45, GROUND_Y - 70, 55);
      drawMosque(offset + tileW * 0.75, GROUND_Y - 90, 65);
    } else if (state.phase === 1) {
      drawMosque(offset + tileW * 0.08, GROUND_Y - 130, 100);
      drawKaabahSilhouette(offset + tileW * 0.28, GROUND_Y - 90);
      drawMosque(offset + tileW * 0.6, GROUND_Y - 110, 85);
    } else {
      ctx.beginPath();
      ctx.ellipse(offset + tileW * 0.2, GROUND_Y, 180, 80, 0, Math.PI, 0);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(offset + tileW * 0.65, GROUND_Y, 200, 60, 0, Math.PI, 0);
      ctx.fill();
    }
  }
}
...
function drawMosque(x, y, scale) {
  ...
  ctx.fillRect(48 * s, -30 * s, 12 * s, 90 * s);
  // Crescent on top of left minaret
...
function gameLoop() {
  ...
  // ALWAYS clear full canvas first — prevents ghost/trail glitches
